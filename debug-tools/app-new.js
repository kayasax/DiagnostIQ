...
class QueryLibraryApp {
    ...
    populateCategoryDropdown() {
        ...
        // Get unique categories from data manager (already normalized)
        const categories = window.dataManager.getCategories();

        // Clear existing options except first
        const defaultOption = categoryDropdown.firstElementChild;
    ...
    populateVerticalDropdown() {
        ...
        // Get unique verticals from data manager
        const verticals = window.dataManager.getVerticals();

        // Clear existing options except first
        const defaultOption = verticalDropdown.firstElementChild;
        verticalDropdown.innerHTML = '';
        if (defaultOption) {
            verticalDropdown.appendChild(defaultOption);
        }

        // Add verticals
        verticals.forEach(vertical => {
            const option = document.createElement('option');
            option.value = vertical.toLowerCase();
            option.textContent = vertical;
            verticalDropdown.appendChild(option);
        });
    }
